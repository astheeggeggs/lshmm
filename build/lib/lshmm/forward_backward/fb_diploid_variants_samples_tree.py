"""WIP: Collection of functions to run forwards and backwards algorithms on diploid genotype data encoded with tree sequences."""
import numpy as np
import tskit
...
class ValueTransition:
    ...
    def __init__(self, tree_1_node=-1, tree_2_node=-1, value=-1, value_index=-1):
        self.tree_1_node = tree_1_node
        self.tree_2_node = tree_2_node
        self.value = value
    ...
    def copy(self):
        ...
        return ValueTransition(
            self.tree_1_node, self.tree_2_node, self.value, self.value_index
        )
...
class LsHmmAlgorithm:
    ...
    def __init__(self, ts, r, mu, precision=10):
        self.ts = ts
        self.mu = mu
        self.r = r
        ...
        for st in T:
            ...
            if u != -1:
                ...
                while allelic_state[v] == -1:
                    ...
                    assert v != -1
                match = (
                    genotype_state == tskit.MISSING_DATA
                    or genotype_state == allelic_state[v]
                )
            allelic_state_1 = allelic_state[v]
            ...
            for st2 in T:
                ...
                if u != -1:
                    ...
                    OBS_HET = np.int64(genotype_state == 1)
                    MATCHES = np.int64(
                        genotype_state == allelic_state_1 + allelic_state[v]
                    )
                    REF_HET = np.int64(allelic_state_1 + allelic_state[v])

                    emission_type = 4 * MATCHES + 2 * REF_HET + OBS_HET

                    if blah is False:
                        st.value = self.compute_next_probability(
                            site.id, st.value, emission_type, u
                        )
                        blah = True
    ...
    def run_forward(self, s):
        ...
        while self.tree.next():
            ...
            for site in self.tree.sites():
                self.process_site(site, s[site.id])
        return self.output
    ...
    def compute_next_probability(self, site_id, p_last, emission_type, node):
...
class ForwardAlgorithm(LsHmmAlgorithm):
    ...
    def __init__(self, ts, r, mu, precision=10):
        super().__init__(ts, r, mu, precision)
    ...
    def compute_next_probability(self, site_id, p_last, emission_type, node):
        ...
        if emission_type == EQUAL_BOTH_HOM:
            p_e = (1 - mu) ** 2
        elif emission_type == UNEQUAL_BOTH_HOM:
            p_e = mu ** 2
        elif emission_type == BOTH_HET:
            p_e = 1 - mu
        elif emission_type == REF_HOM_OBS_HET:
            p_e = 2 * mu * (1 - mu)
        else:
            # REF_HET_OBS_HOM
            p_e = mu * (1 - mu)
        ...
        p_t = ((1 - r) ** 2) * p_last + (r / n) ** 2

        return p_t * p_e


def ls_forward_tree(s, ts, r, mu, precision=30):
    """Forward matrix computation based on a tree sequence."""
    fa = ForwardAlgorithm(ts, r, mu, precision=precision)
    return fa.run_forward(s)
